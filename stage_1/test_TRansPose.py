...
if __name__ == "__main__":
    ...
    if "syn" in opt.dataset:
        indir = os.path.join("../stage_0/SurfEmb/data/bop/", opt.dataset, f"syn_test_{opt.index:02d}/train_pbr")
        outdir = os.path.join("../stage_0/SurfEmb/data/bop/", opt.dataset, f"syn_test_{opt.index:02d}/train_pbr/SurfEmb")

    else:
        indir = os.path.join("../stage_0/SurfEmb/data/bop/", opt.dataset, f"test/seq_test_{opt.index:02d}/cam_R")
        outdir = os.path.join("../stage_0/SurfEmb/data/bop/", opt.dataset, f"test/seq_test_{opt.index:02d}/cam_R/SurfEmb")
                ...
                for j in range(opt.batch):
                    ...
                    for k in range(object_num):
                        if "syn" in opt.dataset:
                            ...
                            object_category = obj_id_info[k]["obj_id"][:8]
                        else:
                            mask_visib_image_path = os.path.join(indir,"mask", f"{image_index:06d}_{(k+1):06d}"+".png")
                            object_category = obj_id_info[k]["obj_name"][:8]
                        try:
                        ...
                        if width <10  or height <10:
                            continue
                        if "N" in object_category:
                            continue
                        text = class_label_json[object_category]
