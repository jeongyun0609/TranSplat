...
class RgbLoader(BopInstanceAux):
    ...
    def __call__(self, inst: dict, dataset: BopInstanceDataset) -> dict:
        ...
        if type(img_id) is int:
            fp = f'{scene_id}/{dataset.img_folder}/{img_id:06d}.{dataset.img_ext}'
        else:
            fp = f'{scene_id}/{dataset.img_folder}/{img_id}.{dataset.img_ext}'
...
class MaskLoader(BopInstanceAux):
    def __init__(self, mask_type='mask'):
        self.mask_type = mask_type

    def __call__(self, inst: dict, dataset: BopInstanceDataset) -> dict:
        scene_id, img_id, pose_idx = inst['scene_id'], inst['img_id'], inst['pose_idx']
        if "pbr" in scene_id:
            self.mask_type = 'mask_visib'
        else:
            self.mask_type = "mask"
        mask_folder = f'{scene_id}/{self.mask_type}'
        ...
        if type(img_id) is int:
            ...
            mask = cv2.imread(str(mask_folder / f'{img_id}_{pose_idx:06d}.png'), cv2.IMREAD_GRAYSCALE)
        assert mask is not None
        inst["mask_visib"] = mask
        return inst
    ...
    def __call__(self, inst: dict, _) -> dict:
        ...
        if self.p.use_bbox:
            ...
            if "mask" in inst.keys():
                mask_arg_rotated = np.argwhere(inst["mask"])[:, ::-1] @ R.T
